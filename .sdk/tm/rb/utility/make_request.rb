...
module JsonIpGeolocationUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = JsonIpGeolocationResponse.new({})
    ...
    if fetch_err
      ...
      response = JsonIpGeolocationResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = JsonIpGeolocationResponse.new(fetched)
    else
