...
//    only - a typed JsonIpGeolocationContext there would make every ctx
...
final class JsonIpGeolocationOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new JsonIpGeolocationContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new JsonIpGeolocationSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new JsonIpGeolocationResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new JsonIpGeolocationError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new JsonIpGeolocationResponse($clean['response']);
    ...
    public static function err_from_map($m): ?JsonIpGeolocationError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new JsonIpGeolocationError(is_string($code) ? $code : '', $msg);
    }
