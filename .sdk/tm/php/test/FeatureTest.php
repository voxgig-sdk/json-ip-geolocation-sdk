...
// JsonIpGeolocation SDK feature test
//
...
class FtCtrl extends JsonIpGeolocationControl
{
...
class FtClient
{
    ...
    private JsonIpGeolocationUtility $utility;

    public function __construct(JsonIpGeolocationUtility $utility, array $options)
    {
    ...
    public function get_utility(): JsonIpGeolocationUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (JsonIpGeolocationContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public JsonIpGeolocationContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (JsonIpGeolocationContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = JsonIpGeolocationConfig::make_feature($name);
    ...
    private static function buildUrl(JsonIpGeolocationSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof JsonIpGeolocationError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new JsonIpGeolocationResponse($fetched);
        }
    ...
    private function fail(JsonIpGeolocationContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (JsonIpGeolocationContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
