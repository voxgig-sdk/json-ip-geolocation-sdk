...
// JsonIpGeolocation SDK retry feature
...
class JsonIpGeolocationRetryFeature extends JsonIpGeolocationBaseFeature
{
    ...
    public function init(JsonIpGeolocationContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (JsonIpGeolocationContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(JsonIpGeolocationContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(JsonIpGeolocationContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
