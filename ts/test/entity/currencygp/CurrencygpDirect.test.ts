...
describe('CurrencygpDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new JsonIpGeolocationSDK({
  ...
  test('direct-load-currencygp', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-currencygp', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['JSON_IP_GEOLOCATION_TEST_CURRENCYGP_ENTID']
