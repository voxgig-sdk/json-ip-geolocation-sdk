...
//     (\`new JsonIpGeolocationSDK(...)\` / \`JsonIpGeolocationSDK.test(...)\` become
//     \`JsonIpGeolocationSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'JsonIpGeolocationSDK'
...
const TEST_SEED = {"entity":{"currencygp":{"test01":{"id":"test01"}},"jsongp":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new JsonIpGeolocationSDK(...)\` and any
// \`JsonIpGeolocationSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+JsonIpGeolocationSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/JsonIpGeolocationSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
