# JsonIpGeolocation Lua SDK



The Lua SDK for the JsonIpGeolocation API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Currencygp()` — each with the same small set of operations (`load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/json-ip-geolocation-sdk/releases)),
...
```lua
local currencygp, err = client:Currencygp():load({ amount = 1, from = "example_from", to = "example_to" })
...
```lua
local currencygp, err = client:Currencygp():load({ amount = 1, from = "example", to = "example" })
if err then error(err) end
```
...
local result, err = client:Currencygp():load({ amount = 1, from = "example", to = "example" })
...
```lua
local currencygp, err = client:Currencygp():load({ amount = 1, from = "from", to = "to" })
```
...
```lua
local jsongp, err = client:Jsongp():load()
```
...
```
lua/
├── json-ip-geolocation_sdk.lua    -- Main SDK module
...
The main module (`json-ip-geolocation_sdk`) exports the SDK constructor
