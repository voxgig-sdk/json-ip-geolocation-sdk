...
The Lua SDK for the JsonIpGeolocation API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Currencygp(nil):load({ id = "example_id" }, nil)
...
local result, err = client:JsonIpGeolocation(nil):load(
  { id = "test01" }, nil
)
