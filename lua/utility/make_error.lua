...
local function make_error_util(ctx, err)
  ...
  local msg = "JsonIpGeolocationSDK: " .. opname .. ": " .. errmsg
  ...
  local sdk_err = JsonIpGeolocationError.new("", msg, ctx)
  sdk_err.result = ctx.utility.clean(ctx, result)
  sdk_err.spec = ctx.utility.clean(ctx, spec)

  if type(err) == "table" and getmetatable(err) == JsonIpGeolocationError then
