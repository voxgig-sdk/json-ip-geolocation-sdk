...
require("json-ip-geolocation_types")
...
local JsonIpGeolocationSDK = {}
JsonIpGeolocationSDK.__index = JsonIpGeolocationSDK
...
JsonIpGeolocationSDK._make_feature = _make_feature


function JsonIpGeolocationSDK.new(options)
  local self = setmetatable({}, JsonIpGeolocationSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function JsonIpGeolocationSDK:options_map()
...
function JsonIpGeolocationSDK:get_utility()
  return Utility.copy(self._utility)
end


function JsonIpGeolocationSDK:get_root_ctx()
  return self._rootctx
end


function JsonIpGeolocationSDK:prepare(fetchargs)
...
function JsonIpGeolocationSDK:direct(fetchargs)
...
function JsonIpGeolocationSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function JsonIpGeolocationSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "JsonIpGeolocationSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function JsonIpGeolocationSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function JsonIpGeolocationSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "JsonIpGeolocationSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function JsonIpGeolocationSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = JsonIpGeolocationSDK.new(sdkopts)
  sdk.mode = "test"
