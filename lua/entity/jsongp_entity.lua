...
local JsongpEntity = {}
JsongpEntity.__index = JsongpEntity


function JsongpEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, JsongpEntity)
  self._name = "jsongp"
  self._client = client
...
function JsongpEntity:get_name()
  return self._name
end


function JsongpEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return JsongpEntity.new(self._client, opts)
end
...
function JsongpEntity:mark_deleted()
  self._deleted = true
end


function JsongpEntity:deleted()
  return true == self._deleted
end


function JsongpEntity:data_set(args)
  if args ~= nil then
...
function JsongpEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function JsongpEntity:match_set(args)
  if args ~= nil then
...
function JsongpEntity:match_get()
...
function JsongpEntity:stream(action, args, callopts)
...
---@param reqmatch JsongpLoadMatch
---@param ctrl? table
---@return Jsongp
---@return string? err
function JsongpEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function JsongpEntity:_run_op(ctx, post_done)
  local utility = self._utility
