# json-ip-geolocation-mcp

[MCP](https://modelcontextprotocol.io) server exposing the JsonIpGeolocation SDK as
two agent tools — `json-ip-geolocation_list` and `json-ip-geolocation_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/json-ip-geolocation-mcp)
make build
...
claude mcp add --scope user json-ip-geolocation \
  -- /absolute/path/to/json-ip-geolocation-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./json-ip-geolocation-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// json-ip-geolocation_list: first page of records
{ "entity": "currencygp" }
{ "entity": "currencygp", "query": { } }

// json-ip-geolocation_load: one record by id
{ "entity": "currencygp", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/json-ip-geolocation-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user json-ip-geolocation \
     -- "$PWD"/dist/*/json-ip-geolocation-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `json-ip-geolocation_list` and `json-ip-geolocation_load` tools now appear
   in new sessions. Ask the agent to *"list currencygp using json-ip-geolocation"*
   and it calls `json-ip-geolocation_list` with `{"entity":"currencygp"}`.
...
### Call the `json-ip-geolocation_list` tool
...
### Call the `json-ip-geolocation_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `json-ip-geolocation_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `json-ip-geolocation_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
```sh
./json-ip-geolocation-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"json-ip-geolocation_load","arguments":{"entity":"currencygp","query":{"id":1}}}}'
```
