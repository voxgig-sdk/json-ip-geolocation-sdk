...
class CurrencygpEntity
  ...
  def make
    opts = @_entopts.dup
    CurrencygpEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = JsonIpGeolocationHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Currencygp, Hash] the current Currencygp data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = JsonIpGeolocationHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Currencygp fields)
  def match_get
  ...
  #
  # @param reqmatch [CurrencygpLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Currencygp.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Currencygp, Hash] the loaded Currencygp; raises JsonIpGeolocationError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = JsonIpGeolocationHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
