...
The Ruby SDK for the JsonIpGeolocation API — an entity-oriented client using idiomatic Ruby conventions.
...
raise a `JsonIpGeolocationError` (a `StandardError` subclass), so wrap
...
| `geoplugin_status` | `Integer` | HTTP status code of the response |
