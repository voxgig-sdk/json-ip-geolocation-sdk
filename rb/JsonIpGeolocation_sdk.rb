...
require_relative 'JsonIpGeolocation_types'
...
class JsonIpGeolocationSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = JsonIpGeolocationHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = JsonIpGeolocationHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, JsonIpGeolocationFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    JsonIpGeolocationUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = JsonIpGeolocationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = JsonIpGeolocationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = JsonIpGeolocationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => JsonIpGeolocationError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    rescue JsonIpGeolocationError => err
    ...
    if fetched.is_a?(Hash)
      status = JsonIpGeolocationHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = JsonIpGeolocationError.new(
        "graphql_error", "JsonIpGeolocationSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = JsonIpGeolocationSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
