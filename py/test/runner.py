# JsonIpGeolocation SDK test runner
...
class JsonIpGeolocationTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            JsonIpGeolocationTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = JsonIpGeolocationTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = JsonIpGeolocationTestRunner.getenv("JSON_IP_GEOLOCATION_TEST_LIVE")
        override = JsonIpGeolocationTestRunner.getenv("JSON_IP_GEOLOCATION_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = JsonIpGeolocationTestRunner.getenv(key)
        ...
        explain = JsonIpGeolocationTestRunner.getenv("JSON_IP_GEOLOCATION_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["JSON_IP_GEOLOCATION_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if JsonIpGeolocationTestRunner._test_control is not None:
            return JsonIpGeolocationTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                JsonIpGeolocationTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            JsonIpGeolocationTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = JsonIpGeolocationTestRunner.load_test_control()
...
def load_env_local():
    JsonIpGeolocationTestRunner.load_env_local()


def env_override(m):
    return JsonIpGeolocationTestRunner.env_override(m)


def entity_data(v):
    return JsonIpGeolocationTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return JsonIpGeolocationTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return JsonIpGeolocationTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return JsonIpGeolocationTestRunner.load_test_control()


def live_client_options():
    return JsonIpGeolocationTestRunner.live_client_options()


def live_delay_ms():
    return JsonIpGeolocationTestRunner.live_delay_ms()
