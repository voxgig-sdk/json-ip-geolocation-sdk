...
from jsonipgeolocation_sdk.jsonipgeolocation_types import (
    Currencygp,
    CurrencygpLoadMatch,
)
...
class CurrencygpEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return CurrencygpEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Currencygp:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Currencygp().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
