...
The JSON IP Geolocation API allows you to determine the geographical location of visitors based on their IP addresses.

Learn more about IP Geolocation & Currency Converter at [www.geoplugin.com/webservices/json](https://www.geoplugin.com/webservices/json).
...
// currencygp is the Currencygp entity, populated with mock data
...
// Load a specific currencygp (returns the ENTITY; call data_get() for the record; throws on error)
$currencygp = $client->Currencygp()->load(["amount" => 1, "from" => "example_from", "to" => "example_to"]);
print_r($currencygp);
```
...
# Load a specific currencygp (returns the ENTITY; call data_get for the record)
currencygp = client.Currencygp.load({ "amount" => 1, "from" => "example_from", "to" => "example_to" })
puts currencygp
```
