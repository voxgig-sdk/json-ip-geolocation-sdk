...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("JSONIPGEOLOCATION_TEST_LIVE") == "TRUE" ||
		os.Getenv("JSONIPGEOLOCATION_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("JSONIPGEOLOCATION_TEST_EXPLAIN"); explain != "" {
