...
func TestCurrencygpEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set JSON_IP_GEOLOCATION_TEST_CURRENCYGP_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		currencygpRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.currencygp")))
...
func currencygpBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("JSON_IP_GEOLOCATION_TEST_CURRENCYGP_ENTID")
	...
	idmapResolved := core.ToMapAny(env["JSON_IP_GEOLOCATION_TEST_CURRENCYGP_ENTID"])
