...
import (
	...
	omni "github.com/voxgig-sdk/json-ip-geolocation-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.JsonIpGeolocationError); is {
				out := map[string]any{
					"name":    "JsonIpGeolocationError",
...
func omniCtx(arg any, client *sdk.JsonIpGeolocationSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.JsonIpGeolocationSDK); is {
			client = live
