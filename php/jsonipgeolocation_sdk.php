...
class JsonIpGeolocationSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = JsonIpGeolocationHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = JsonIpGeolocationHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!JsonIpGeolocationFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, JsonIpGeolocationFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return JsonIpGeolocationUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = JsonIpGeolocationHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = JsonIpGeolocationHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = JsonIpGeolocationHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new JsonIpGeolocationError($op . "_allow",
                "JsonIpGeolocationSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = JsonIpGeolocationHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new JsonIpGeolocationError("graphql_error",
                "JsonIpGeolocationSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Currencygp($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_currencygp === null) {
                $this->_currencygp = new CurrencygpEntity($this, null);
            }
            return $this->_currencygp;
        }
        return new CurrencygpEntity($this, $data);
    }
    ...
    public function Jsongp($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_jsongp === null) {
                $this->_jsongp = new JsongpEntity($this, null);
            }
            return $this->_jsongp;
        }
        return new JsongpEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new JsonIpGeolocationSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
