...
// JsonIpGeolocation SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = JsonIpGeolocationOmni::makeStructRunner(
    ...
    {
        return JsonIpGeolocationOmni::objmodel(JsonIpGeolocationOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return JsonIpGeolocationOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return JsonIpGeolocationOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(JsonIpGeolocationOmni::structfix($now, Struct::undef()))
                        !== json_encode(JsonIpGeolocationOmni::structfix($val, Struct::undef()))) {
