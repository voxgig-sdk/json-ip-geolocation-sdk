...
// JsonIpGeolocation SDK pipeline test
//
...
{
    private static function utility(): JsonIpGeolocationUtility
    {
        return new JsonIpGeolocationUtility();
    }
    ...
    private static function ctx(array $over = []): JsonIpGeolocationContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new JsonIpGeolocationContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new JsonIpGeolocationOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof JsonIpGeolocationError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new JsonIpGeolocationOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = JsonIpGeolocationMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new JsonIpGeolocationOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = JsonIpGeolocationMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new JsonIpGeolocationOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = JsonIpGeolocationMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new JsonIpGeolocationSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = JsonIpGeolocationMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new JsonIpGeolocationResponse([]),
            'result' => new JsonIpGeolocationResult([]),
        ]);
        [, $err] = JsonIpGeolocationMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new JsonIpGeolocationSpec([]),
            'response' => null,
            'result' => new JsonIpGeolocationResult([]),
        ]);
        [, $err] = JsonIpGeolocationMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new JsonIpGeolocationSpec(['step' => 's']),
            'response' => new JsonIpGeolocationResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new JsonIpGeolocationResult([]),
        ]);
        [, $err] = JsonIpGeolocationMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new JsonIpGeolocationSpec(['step' => 's']),
            'response' => new JsonIpGeolocationResponse(self::resp(200, ['v' => 1])),
            'result' => new JsonIpGeolocationResult([]),
        ]);
        [, $err] = JsonIpGeolocationMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new JsonIpGeolocationSpec(['step' => 's']),
            'response' => new JsonIpGeolocationResponse(self::resp(200, ['v' => 2])),
            'result' => new JsonIpGeolocationResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        JsonIpGeolocationMakeResponse::call($ctx);
    ...
    {
        $preset = new JsonIpGeolocationResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new JsonIpGeolocationSpec([]),
            'response' => new JsonIpGeolocationResponse([]),
            'result' => new JsonIpGeolocationResult([]),
        ...
        [$got, $err] = JsonIpGeolocationMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new JsonIpGeolocationResult([])]);
        [, $err] = JsonIpGeolocationMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new JsonIpGeolocationSpec([]), 'result' => null]);
        [, $err] = JsonIpGeolocationMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new JsonIpGeolocationSpec(['step' => 's']),
            'result' => new JsonIpGeolocationResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new JsonIpGeolocationOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = JsonIpGeolocationMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new JsonIpGeolocationSpec(['step' => 's']),
            'result' => new JsonIpGeolocationResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new JsonIpGeolocationOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = JsonIpGeolocationMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new JsonIpGeolocationResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new JsonIpGeolocationSpec([]),
            'result' => new JsonIpGeolocationResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = JsonIpGeolocationMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = JsonIpGeolocationMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new JsonIpGeolocationError('boom', 'boom');
        $utility->fetcher = function (JsonIpGeolocationContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new JsonIpGeolocationSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = JsonIpGeolocationMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (JsonIpGeolocationContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = JsonIpGeolocationMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(JsonIpGeolocationResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        JsonIpGeolocationMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (JsonIpGeolocationContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new JsonIpGeolocationSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = JsonIpGeolocationMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new JsonIpGeolocationResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new JsonIpGeolocationSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = JsonIpGeolocationMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = JsonIpGeolocationMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = JsonIpGeolocationMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new JsonIpGeolocationResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, JsonIpGeolocationDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new JsonIpGeolocationResult(['ok' => false])]);
        $this->expectException(JsonIpGeolocationError::class);
        JsonIpGeolocationDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new JsonIpGeolocationResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', JsonIpGeolocationMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        JsonIpGeolocationMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new JsonIpGeolocationBaseFeature();
        $b = new JsonIpGeolocationBaseFeature();
        JsonIpGeolocationFeatureAdd::call($ctx, $a);
        JsonIpGeolocationFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): JsonIpGeolocationBaseFeature
    {
        $f = new JsonIpGeolocationBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        JsonIpGeolocationFeatureAdd::call($ctx, self::named_feature('a'));
        JsonIpGeolocationFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        JsonIpGeolocationFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        JsonIpGeolocationFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        JsonIpGeolocationFeatureAdd::call($ctx, $replace);
        ...
        JsonIpGeolocationFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): JsonIpGeolocationContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new JsonIpGeolocationSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = JsonIpGeolocationPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        JsonIpGeolocationPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new JsonIpGeolocationResponse(['headers' => null]),
            'result' => new JsonIpGeolocationResult([]),
        ]);
        JsonIpGeolocationResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        JsonIpGeolocationResultBody::call($ctx);
