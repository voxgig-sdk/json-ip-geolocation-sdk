...
// JsonIpGeolocation SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class JsonIpGeolocationTestFeature extends JsonIpGeolocationBaseFeature
{
    ...
    public function init(JsonIpGeolocationContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (JsonIpGeolocationContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = JsonIpGeolocationParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (JsonIpGeolocationContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(JsonIpGeolocationContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = JsonIpGeolocationParam::call($ctx, $k);
